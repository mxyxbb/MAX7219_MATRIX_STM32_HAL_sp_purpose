...
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC), ms, MAX7219_MatrixSetPixel, MAX7219_MatrixSetPixel(index,digit,row));
...
void MAX7219_MatrixRShift(uint8_t step)
{
	...
	for(int i=0; i < 8; i++)
	{
		msbD0 = ((FrameBuffer[2][i] & 0x80) >> 7);
		FrameBuffer[0][i] = FrameBuffer[0][i] << 1;

		msbD1 = ((FrameBuffer[3][i] & 0x80) >> 7);
		FrameBuffer[1][i] = FrameBuffer[1][i] << 1;
		FrameBuffer[1][i] |= msbD0;

		msbD2 = ((FrameBuffer[0][i] & 0x80) >> 7);
		FrameBuffer[2][i] = FrameBuffer[2][i] << 1;
		FrameBuffer[2][i] |= msbD1;

		msbD3 = ((FrameBuffer[1][i] & 0x80) >> 7);
		FrameBuffer[3][i] = FrameBuffer[3][i] << 1;
		FrameBuffer[3][i] |= msbD2;
		FrameBuffer[0][i] |= msbD3;
	}
}
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC), mr, MAX7219_MatrixRShift, MAX7219_MatrixRShift(step));
...
void MAX7219_MatrixLShift(uint8_t step)
{
	...
	for(int i=0; i < 8; i++)
	{
		lsbD3 = ((FrameBuffer[1][i] & 0x01) << 7);
		FrameBuffer[3][i] = FrameBuffer[3][i] >> 1;

		lsbD2 = ((FrameBuffer[0][i] & 0x01) << 7);
		FrameBuffer[2][i] = FrameBuffer[2][i] >> 1;
		FrameBuffer[2][i] |= lsbD3;

		lsbD1 = ((FrameBuffer[3][i] & 0x01) << 7);
		FrameBuffer[1][i] = FrameBuffer[1][i] >> 1;
		FrameBuffer[1][i] |= lsbD2;

		lsbD0 = ((FrameBuffer[2][i] & 0x01) << 7);
		FrameBuffer[0][i] = FrameBuffer[0][i] >> 1;
		FrameBuffer[0][i] |= lsbD1;
		FrameBuffer[3][i] |= lsbD0;
	}
}
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC), ml, MAX7219_MatrixLShift, MAX7219_MatrixLShift(step));
...
void MAX7219_mywrite(uint8_t *numstr)
{
	...
	//display first two number
	uint64_t num1=numbers3_5[numstr[0]];
	uint64_t num2=numbers3_5[numstr[1]];
	for(i=0;i<8;i++){
		FrameBuffer[0][6-i] = (num1<<5)|((num2&0x0f)<<1);
		num1=num1>>8;
		num2=num2>>8;
	}
	//display the third number and the sign '+'
	num1=numbers3_5[numstr[2]];
	num2=numbers3_5[4];// sign '+'
//	for(i=0;i<8;i++){
//		FrameBuffer[1][6-i] = (num1<<5)|((num2&0x1f));
//		num1=num1>>8;
//		num2=num2>>8;
//	}
	for(i=0;i<8;i++){
		FrameBuffer[1][6-i] = (num1<<5)|((num2&0x1f)>>2);
		FrameBuffer[2][6-i] = ((num2&0x03)<<6);
		num1=num1>>8;
		num2=num2>>8;
	}
	//display the fourth number
	num1=numbers3_5[numstr[3]];
	for(i=0;i<8;i++){
		FrameBuffer[2][6-i] |= ((num1&0x0f)<<1);
		num1=num1>>8;
	}
	//display the fifth and sixth number
	num1=numbers3_5[numstr[4]];
	num2=numbers3_5[numstr[5]];
	for(i=0;i<8;i++){
		FrameBuffer[3][6-i] = (num1<<5)|((num2&0x0f)<<1);
		num1=num1>>8;
		num2=num2>>8;
	}
	
}
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC), mm, MAX7219_mywrite, MAX7219_mywrite(*num));
